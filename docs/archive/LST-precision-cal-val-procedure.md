...
In general, these plots show that the median CE95 values are below the 1 pixel accuracy threshold over the operational time for both the TIR camera (30m GSD) and the VNIR camera (10m GSD).  
...
The comparison of the surface temperatures measured by SkyBee-1 and Skybee-2 with reference ground data in the near neighbourhood of key validation sites demonstrates the high accuracy of the [LSTprecision product](https://constellr.github.io/product-lst/archive/LST-precision/) both during the day and at night, as shown in Figure 3. Each data point collected over those reference sites during this early phase of the mission matches nicely the reference measurements with a mean deviation being close to only 1 degree. Such a high accuracy, though challenging to reach, offers opportunities for a large range of use cases based on the detection of thermal anomalies.
...
<figcaption>Figure 3: Comparison of surface temperatures measured from space by SkyBee-01 with high-quality reference ground data over a few key validation sites. We gratefully acknowledge the <a href= https://gml.noaa.gov/grad/surfrad/ >SURFRAD</a>, <a href= https://calval.jpl.nasa.gov/ >JPL data portals</a>, and the <a href= https://law.acri-st.fr/home >Copernicus LAW portal</a> as well as the colleagues from KIT-IMKASF for providing the in-situ data.</figcaption>
